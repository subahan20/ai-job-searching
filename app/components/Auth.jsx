...
export default function Auth({ onAuthSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (activeTab === 'signup') {
      ...
      if (!agreedToTerms) { setErrorMsg('Please agree to the Terms of Service.'); return; }
    }
  ...
  const inputCls = "w-full border border-zinc-200 rounded-lg px-3.5 py-2.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-[#22c55e] focus:ring-2 focus:ring-[#22c55e]/20 transition-all bg-white";
      ...
      <div className="w-full max-w-[480px] h-[min(720px,calc(100dvh-5rem))] flex flex-col bg-white rounded-2xl shadow-lg overflow-hidden shrink-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto auth-scroll-hidden px-10 pt-4 pb-5 flex flex-col items-center">
          ...
          <form onSubmit={handleSubmit} autoComplete="off" className="w-full flex flex-col gap-3">
            ...
            {activeTab === 'signup' && (
              <div>
                <label htmlFor="full-name" className="text-[12px] font-semibold text-zinc-700 block mb-1">Full Name</label>
                <input id="full-name" type="text" required autoComplete="off" value={fullName}
                  onChange={(e) => setFullName(e.target.value)} placeholder="John Doe" className={inputCls} />
              </div>
            )}

            {/* Email */}
            <div>
              <label htmlFor="auth-email" className="text-[12px] font-semibold text-zinc-700 block mb-1">Email Address</label>
              <input id="auth-email" type="email" required autoComplete="off" value={email}
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <label htmlFor="auth-password" className="text-[12px] font-semibold text-zinc-700">Password</label>
                {activeTab === 'login' && (
              ...
              <div className="relative">
                <input id="auth-password" type={showPassword ? 'text' : 'password'} required
            ...
            {/* Confirm Password & Terms — signup only */}
            {activeTab === 'signup' && (
              <>
                <div>
                  <label htmlFor="confirm-password" className="text-[12px] font-semibold text-zinc-700 block mb-1">Confirm Password</label>
                  <div className="relative">
                ...
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <span className="text-[11px] text-zinc-500 leading-snug">
                    I agree to the{' '}
                    <a href="#" className="text-[#22c55e] font-semibold hover:underline">Terms of Service</a>
                    {' '}and{' '}
                    <a href="#" className="text-[#22c55e] font-semibold hover:underline">Privacy Policy</a>.
                  </span>
                </label>
              </>
            )}
          ...
          <p className="text-[13px] text-zinc-400 mt-4 text-center">
            {activeTab === 'login' ? (
              <>Don&apos;t have an account?{' '}
                <button type="button" onClick={() => switchTab('signup')} className="text-[#22c55e] font-bold hover:underline">
                  Register
                </button>
              </>
            ) : (
              <>Already have an account?{' '}
                <button type="button" onClick={() => switchTab('login')} className="text-[#22c55e] font-bold hover:underline">
                  Login
                </button>
              </>
            )}
        ...
        {/* Rainbow strip */}
        <div className="h-1 w-full shrink-0" style={{ background: 'linear-gradient(to right,#ef4444,#f97316,#eab308,#22c55e,#3b82f6,#8b5cf6)' }} aria-hidden="true" />
      </div>
    </div>
  );
}
