...
export default function AiJobsBrowsePage() {
  ...
  const { jobs, loading, error } = useAiSearchJobsBrowse({ sortBy });
          ...
          <div className="mb-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
              ...
              <p className="text-[12px] text-zinc-500 mt-1 font-semibold">
                {loading
                  ? 'Loading jobs...'
                  : `Showing ${jobs.length} jobs posted in the last 7 days from LinkedIn, Naukri & Indeed.`}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mb-5 flex-1">
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64 text-zinc-400 text-sm font-semibold">
                No AI-searched jobs yet. Run a search from the Dashboard to discover jobs.
              </div>
            )}
          </div>
