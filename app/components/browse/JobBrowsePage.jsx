...
export default function JobBrowsePage({ title, subtitle, source, filterFn }) {
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedJob, setSelectedJob] = useState(null);
  const [sortBy, setSortBy] = useState('latest');
  const { jobs, loading, error } = useBrowseJobs({ source, filterFn, sortBy });
      ...
      <div className="flex flex-col h-full bg-[#f8f9fc]">
        <main className="flex-1 px-8 py-6 flex flex-col">
          <div className="mb-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
              <h2 className="text-[20px] font-black text-zinc-900 tracking-tight">{title}</h2>
              <p className="text-[12px] text-zinc-500 mt-1 font-semibold">
                {loading ? 'Loading jobs...' : subtitle(jobs.length)}
              </p>
            </div>
            <button
              type="button"
